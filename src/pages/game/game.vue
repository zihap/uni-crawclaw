                ...
                <view class="modal-body" v-if="battleP1 && battleP2" style="background: #f8fafc">
                    ...
                    <view class="battle-vs-header">
                        <view class="vs-player p1">
                            ...
                            <text class="vs-res"
                                >🌿{{ battleP1.seaweed }} 🪙{{ battleP1.coins }} 🎖{{ battleP1.de }}</text
                            >
                        </view>
                        ...
                        <view class="vs-player p2" style="align-items: flex-end">
                            ...
                            <text class="vs-res"
                                >🎖{{ battleP2.de }} 🪙{{ battleP2.coins }} 🌿{{ battleP2.seaweed }}</text
                            >
                        </view>
                    </view>
                    ...
                    <view v-if="battleState.phase === 'SELECT'" style="margin-top: 1rem">
                        ...
                        <view class="lobster-grid" style="margin-top: 0.5rem; grid-template-columns: repeat(2, 1fr)">
                            <view
                                v-for="(lobster, idx) in battleSelectableLobsters"
                                :key="idx"
                                class="lobster-card"
                                :class="{ 'max-royal': isLobsterBanned(lobster) }"
                            ...
                            >
                                ...
                                <view v-if="isLobsterBanned(lobster)" class="max-grade-mask" style="background: #ef4444"
                                    >{{ lobster.hasFought ? '已参战' : '不可战' }}
                                </view>
                            </view>
                        </view>
                        <view v-if="battleSelectableLobsters.length === 0" class="empty-hint"
                            >该玩家没有可出战的龙虾！
                        </view>
                    </view>
                        ...
                        <view class="battle-action-box mt-4">
                            ...
                            <view class="grass-toggle-container">
                                ...
                                <view class="grass-options">
                                    <text
                                        class="grass-opt"
                                        :class="{ active: battleState.grassToUse === 0 }"
                                        @click="setBattleGrass(0)"
                                        >不使用
                                    </text>
                                    <text
                                        class="grass-opt"
                                        :class="{
                                            active: battleState.grassToUse === 1,
                                            disabled: battleMaxGrassAllowed < 1
                                        }"
                                        @click="setBattleGrass(1)"
                                        >用 1 株
                                    </text>
                                </view>
                            </view>

                            <text v-if="battleMaxGrassAllowed === 0" class="error-hint block text-center mb-2"
                                >⚠️
                            ...
                            <button
                                class="btn w-full roll-action-btn"
                    ...
                    <view v-if="battleState.phase === 'RESULT'" style="margin-top: 1.5rem; text-align: center">
                        <text
                            class="result-title block mb-2"
                            style="font-size: 1.8rem; font-weight: 900"
                            :style="{
                                color:
                        ...
                        >
                            {{
                                battleState.winner === null
                                    ? '平局/异常'
                                    : `${battleState.winner === 1 ? battleP1.name : battleP2.name} 获胜！`
                            }}
                        </text>
                        ...
                        <view v-if="battleState.winner !== null" class="reward-options flex flex-col gap-3">
                            <text class="text-sm font-bold text-gray-700 block text-center mb-2"
                                >请选择决斗战利品：
                            </text>
                            <button
                                ...
                                style="
                                    ...
                                    color: white;
                                "
                                @click="claimBattleReward('UPGRADE')"
                            >
                                ✨ 参战龙虾升 1 阶 (最高皇家)
                            </button>
                            <button
                                ...
                                style="
                                    ...
                                    color: white;
                                "
                                @click="claimBattleReward('COIN')"
                            >
                                🪙 拿取 3 枚金币
                            </button>
                        </view>
                        <button v-else class="btn btn-secondary w-full mt-4" @click="finishBattle">
                            结束并处理席位
                        </button>
                    </view>
                </view>
                ...
                <view class="modal-body">
                    ...
                    <view v-else class="breeding-action-panel">
                        ...
                        <view v-if="isUpgradingToRoyal" class="royal-requirements animate-fade-in">
                            ...
                            <view
                                v-if="gameStore.gameTitleCards.length > 0"
                ...
                <view class="modal-body">
                    ...
                    >
                        ...
                        <view class="mp-options">
                            <view
                                v-for="(opt, idx) in marketplaceState.selectedCard.action?.options || []"
                                :key="idx"
                                class="mp-option-btn"
                            ...
                            >
                                <view
                                    class="custom-radio"
                                    :class="{ checked: marketplaceState.selectedOptionIndex === idx }"
                                ></view>
                                <!-- 运用通用格式化函数展示转换文本 -->
        ...
        <view class="modal-overlay" v-if="gameStore.pendingSeafoodMarket">
            <view class="modal-content seafood-market-modal">
                        ...
                        <view class="stall stall-1">
                            ...
                            <view class="stall-spaces">
                                <view
                                    ...
                                    :key="'s1' + i"
                                    class="stall-space"
                                    :class="{ filled: isSpaceFilled(i - 1) }"
                                ></view>
                            </view>
                        </view>
                        <!-- 摊位2 -->
                        <view class="stall stall-2">
                            ...
                            <view class="stall-spaces">
                                <view
                                    ...
                                    :key="'s2' + i"
                                    class="stall-space"
                                    :class="{ filled: isSpaceFilled(3 + i - 1) }"
                                ></view>
                            </view>
                        </view>
                        <!-- 摊位3 -->
                        <view class="stall stall-3">
                            ...
                            <view class="stall-spaces">
                                <view
                                    ...
                                    :key="'s3' + i"
                                    class="stall-space"
                                    :class="{ filled: isSpaceFilled(5 + i - 1) }"
                                ></view>
                            </view>
                        </view>
                    </view>
                ...
                <view class="modal-tabs">
                    <view class="tab-item" :class="{ active: smTab === 'trade' }" @click="smTab = 'trade'"
                        >资源买卖</view
                    >
                ...
                <view class="modal-body" style="padding-top: 0">
                    ...
                    <view v-if="smTab === 'trade'" class="trade-grid animate-fade-in">
                        <view class="trade-row">
                            <button
                                ...
                                :disabled="
                                    gameStore.seafoodMarketLobsters === 0 ||
                                    currentPendingSeafoodMarket.player.coins < currentMarketPrices.lobster
                                "
                            >
                            ...
                            <button
                                ...
                                :disabled="
                                    gameStore.seafoodMarketLobsters === 8 ||
                                    currentPendingSeafoodMarket.player.lobsters.length === 0
                                "
                            >
                        ...
                        <view class="trade-row">
                            <button
                                class="btn btn-outline"
                                @click="doSeafoodTrade('buy_cage')"
                                :disabled="currentPendingSeafoodMarket.player.coins < currentMarketPrices.cage"
                            >
                                买入虾笼 (-{{ currentMarketPrices.cage }}金)
                            </button>
                            <button
                                class="btn btn-outline"
                                @click="doSeafoodTrade('sell_cage')"
                                :disabled="currentPendingSeafoodMarket.player.cages === 0"
                            >
                                卖出虾笼 (+{{ currentMarketPrices.cage }}金)
                            </button>
                        </view>
                        <view class="trade-row">
                            <button
                                class="btn btn-outline"
                                @click="doSeafoodTrade('buy_seaweed1')"
                            ...
                            <button
                                class="btn btn-outline"
                                @click="doSeafoodTrade('sell_seaweed1')"
                                :disabled="currentPendingSeafoodMarket.player.seaweed < 1"
                            >
                                卖1草 (+{{ currentMarketPrices.seaweed1 }}金)
                            </button>
                        </view>
                        <view class="trade-row">
                            <button
                                class="btn btn-outline"
                                @click="doSeafoodTrade('buy_seaweed3')"
                            ...
                            <button
                                class="btn btn-outline"
                                @click="doSeafoodTrade('sell_seaweed3')"
                                :disabled="currentPendingSeafoodMarket.player.seaweed < 3"
                            >
                                卖3草 (+{{ currentMarketPrices.seaweed3 }}金)
                            </button>
                        </view>
                    </view>
                    ...
                    <view v-if="smTab === 'hire'" class="hire-grid animate-fade-in">
                        ...
                        <view class="slots-container">
                            <view
                                v-for="(slot, idx) in HIRE_LIZHANG_SLOTS"
                                :key="idx"
                                class="hire-slot"
                                :class="{
                                    locked: gameStore.currentRound < slot.availableFrom,
                                    hired: gameStore.hiredLiZhangSlots[idx] !== null
                                }"
                            ...
                            >
                                ...
                                <view class="status-mask" v-if="gameStore.hiredLiZhangSlots[idx] !== null">
                                    <text
                                        v-if="
                                            gameStore.hiredLiZhangSlots[idx] === currentPendingSeafoodMarket.player.id
                                        "
                                    >
                                        已归你
                                    </text>
                ...
                <view class="modal-footer">
                    <button class="btn btn-secondary w-full" @click="skipSeafoodMarketAction">
                        放弃剩余次数并结束
                    </button>
                </view>
            </view>
        ...
        <view class="modal-overlay" v-if="gameStore.pendingTribute">
            <view class="modal-content tribute-modal">
                    ...
                    <view class="taverns-list">
                        <view v-for="tavern in gameStore.taverns" :key="tavern.id" class="tavern-box">
                            <view class="tavern-header">
                                ...
                                <text
                                    class="tavern-status"
                                    v-if="tavern.occupants.includes(currentPendingTribute.player.id)"
                                    >已占席位
                                </text>
                            </view>
                            ...
                            <view class="tribute-cards">
                                <view v-for="card in tavern.cards" :key="card.id" class="tribute-card">
                                    ...
                                    <button
                                        ...
                                        :disabled="
                                            !checkTributeReq(card.requirements, currentPendingTribute.player) ||
                                            tavern.occupants.includes(currentPendingTribute.player.id)
                                        "
                ...
                <view class="modal-body naked-mode-panel animate-fade-in" v-else>
                    ...
                    <view class="lobster-grid">
                        <view
                            v-for="(lobster, index) in getValidNakedLobsters(currentPendingTribute.player)"
                            :key="lobster.id"
                            class="lobster-card"
                    ...
                    <view
                        v-if="getValidNakedLobsters(currentPendingTribute.player).length === 0"
                    ...
                    <view class="tavern-select-grid">
                        <view
                            v-for="t in gameStore.taverns"
                            :key="t.id"
                            class="tavern-select-btn"
                            :class="{
                                active: tributeState.nakedTavernId === t.id,
                                disabled: t.occupants.includes(currentPendingTribute.player.id)
                            }"
                            @click="
                                !t.occupants.includes(currentPendingTribute.player.id) &&
                                (tributeState.nakedTavernId = t.id)
                            "
                        >
                            <text class="ts-name">{{ t.name }}</text>
                            <text class="ts-status">席位: {{ t.occupants.length }}/4</text>
                            <text v-if="t.occupants.includes(currentPendingTribute.player.id)" class="ts-lock"
                                >已占
                            </text>
                        </view>
                    </view>
...
const battleSelectableLobsters = computed(() =>
    battleState.selectPhase === 1 ? battleP1.value?.lobsters || [] : battleP2.value?.lobsters || []
)
const battleCurrentFighter = computed(() =>
    battleState.currentTurn === 1 ? battleState.p1Fighter : battleState.p2Fighter
)
const battleOpponentFighter = computed(() =>
    battleState.currentTurn === 1 ? battleState.p2Fighter : battleState.p1Fighter
)
...
const startBattleMode = () => {
    ...
    battleState.p1Started =
        battleState.p1Fighter.title?.skill?.startStarted || battleState.p1Fighter.title?.name === '急先锋龙虾'
    battleState.p2Started =
        battleState.p2Fighter.title?.skill?.startStarted || battleState.p2Fighter.title?.name === '急先锋龙虾'
    ...
    addBattleLog(
        `--- 轮到 ${battleState.currentTurn === 1 ? '攻方: ' + battleP1.value.name : '守方: ' + battleP2.value.name} 行动 ---`,
        'system'
    )
}
...
const doBattleRollDice = () => {
    ...
    if (battleExpectedGrassBonus.value > 0) {
        addBattleLog(
            `${playerTag} ${consumedGrassText}，基础 ${baseRoll} + 草加成 ${battleExpectedGrassBonus.value} = ${totalRoll} 点！`,
            logType
        )
    } else {
...
const processBattleMovement = (baseRoll, totalRoll, playerName, playerTag, logType) => {
    ...
    if (!wasAlreadyStarted) {
        ...
        if (totalRoll >= 6) {
            ...
            if (isP1) battleState.p1StartRoll = 0
            else battleState.p2StartRoll = 0
        }
    ...
    let newPos = oldPos
    if (isP1) newPos += steps
    else newPos -= steps
    if (isP1) battleState.p1Pos = newPos
    else battleState.p2Pos = newPos
...
const checkBattleWinCondition = () => {
    ...
    if (battleState.currentTurn === 1) {
        if (battleState.p1Pos >= battleState.p2Pos) {
            ...
            (battleState.p1Fighter.title?.skill?.nearWinOnAdjacent || battleState.p1Fighter.title?.name === '长鳌虾') &&
            battleState.p2Pos - battleState.p1Pos <= 1
        ) {
        ...
        if (battleState.p2Pos <= battleState.p1Pos) {
            ...
            (battleState.p2Fighter.title?.skill?.nearWinOnAdjacent || battleState.p2Fighter.title?.name === '长鳌虾') &&
            battleState.p1Pos - battleState.p2Pos <= 1
        ) {
        ...
        if (battleState.currentTurn === 2) {
            ...
            if (battleState.p1Started && battleState.p2Started) {
                ...
                if (battleState.p1StartRoll > battleState.p2StartRoll) {
                    battleState.currentTurn = 1
                    addBattleLog(
                        `双方均兴奋！攻方点数(${battleState.p1StartRoll}) > 守方(${battleState.p2StartRoll})，攻方夺得移动先手！`,
                        'system'
                    )
                } else {
                    battleState.currentTurn = 2
                    addBattleLog(
                        `双方均兴奋！攻方点数(${battleState.p1StartRoll}) ≤ 守方(${battleState.p2StartRoll})，守方夺得移动先手！`,
                        'system'
                    )
                }
...
const formatHireReward = (reward) => {
    ...
    if (reward.lobster)
        return `1只${getLobsterGradeName(LOBSTER_GRADES[reward.lobster.toUpperCase()] || LOBSTER_GRADES.NORMAL)}`
    return '无'
}
